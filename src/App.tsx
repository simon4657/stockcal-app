...
import { AIAnalysisModal } from './components/AIAnalysis';
import {
...
const HotView = ({ hotTrends, onAnalyze }: { hotTrends: any[]; onAnalyze: (trend: any) => void }) => {
  return (
    <div className="flex-1 overflow-y-auto p-4 pb-24 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="space-y-4">
        {hotTrends.map((sector, idx) => (
          <div key={sector.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 relative overflow-hidden hover:border-orange-500/30 transition-colors group">
            ...
            <button
              ...
              className="w-full mt-4 bg-blue-600 hover:bg-blue-500 text-white font-medium py-2 rounded-lg flex items-center justify-center gap-2 transition-colors text-sm"
            >
              <Sparkles size={16} /> AI 深度分析
            </button>
          </div>
        ))}
      </div>
...
const StrategyView = ({ strategies, onAnalyze }: { strategies: any[]; onAnalyze: (strategy: any) => void }) => {
  return (
    <div className="flex-1 overflow-y-auto p-4 pb-24 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-white flex items-center gap-2">
      ...
      <div className="grid gap-4">
        {strategies.map((s) => (
          <div key={s.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 hover:border-slate-600 transition-colors relative overflow-hidden group">
            ...
            <button
              onClick={() => onAnalyze(s)}
              className="w-full mt-3 bg-blue-600 hover:bg-blue-500 text-white font-medium py-2 rounded-lg flex items-center justify-center gap-2 transition-colors text-sm relative z-10"
            >
              <Sparkles size={16} /> AI 深度分析
            </button>
    ...
    <div className="flex flex-col h-screen bg-slate-950 font-sans text-slate-200 overflow-hidden selection:bg-blue-500/30">
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
        ...
        {activeTab === 'hot' && (
          <HotView
            hotTrends={hotTrends}
            onAnalyze={(trend) => setAiAnalysisModal({ type: 'hot-trend', itemId: trend.id, itemData: trend })}
          />
        )}
        {activeTab === 'strategy' && (
          <StrategyView
            strategies={strategies}
            onAnalyze={(strategy) => setAiAnalysisModal({ type: 'strategy', itemId: strategy.id, itemData: strategy })}
          />
        )}
          ...
          >
            ...
            <div className="space-y-6">
              ...
              <button
                ...
                className="w-full bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-white font-bold py-4 rounded-xl shadow-lg shadow-purple-900/30 flex items-center justify-center gap-2 transition-all active:scale-95"
              >
                <Sparkles size={20} /> 開啟 AI 深度分析
              </button>
      ...
      {aiAnalysisModal && (
        <AIAnalysisModal
          type={aiAnalysisModal.type}
          itemId={aiAnalysisModal.itemId}
          itemData={aiAnalysisModal.itemData}
